...
def all_data():
    states={"andaman-&-nicobar-islands": "Andaman & Nicobar","andhra-pradesh": "Andhra Pradesh","arunachal-pradesh": "Arunachal Pradesh","assam": "Assam",
    "bihar": "Bihar","chandigarh": "Chandigarh","chhattisgarh": "Chhattisgarh","dadra-&-nagar-haveli-&-daman-&-diu": "Dadra and Nagar Haveli and Daman and Diu","delhi": "Delhi","goa": "Goa","gujarat": "Gujarat","haryana": "Haryana","himachal-pradesh": "Himachal Pradesh",
    "jammu-&-kashmir": "Jammu & Kashmir","jharkhand": "Jharkhand","karnataka": "Karnataka","kerala": "Kerala","ladakh": "Ladakh","lakshadweep": "Lakshadweep","madhya-pradesh": "Madhya Pradesh",
    "maharashtra": "Maharashtra","manipur": "Manipur","meghalaya": "Meghalaya","mizoram": "Mizoram","nagaland": "Nagaland","odisha": "Odisha","puducherry": "Puducherry","punjab": "Punjab","rajasthan": "Rajasthan",
    "sikkim": "Sikkim","tamil-nadu": "Tamil Nadu","telangana": "Telangana","tripura": "Tripura","uttar-pradesh": "Uttar Pradesh","uttarakhand": "Uttarakhand","west-bengal": "West Bengal"}
    
    #Create a DataFrame agg_trans
    agg_trans = pd.DataFrame( aggregated_transaction())
    agg_trans['State']=agg_trans['State'].replace(states)

    #Create a DataFrame agg_users
    agg_user = pd.DataFrame( aggregated_user())
    agg_user['State']=agg_user['State'].replace(states)

    #Create a DataFrame  map_trans
    map_trans = pd.DataFrame( map_transaction())
    map_trans['State']=map_trans['State'].replace(states)

     #Create a DataFrame  df_map_user 
    df_map_user = pd.DataFrame(map_user())
    df_map_user['State']=df_map_user['State'].replace(states)

    #Create a DataFrame   top_tran_district
    top_tran_district=pd.DataFrame(top_transaction_district())
    top_tran_district['State']=top_tran_district['State'].replace(states)

    #Create a DataFrame    top_trans_pincode
    top_trans_pincode = pd.DataFrame(top_transaction_pincode())
    top_trans_pincode['State']=top_trans_pincode['State'].replace(states)

    top_users_district = pd.DataFrame(top_user_district())
    top_users_district['State']=top_users_district['State'].replace(states)

    top_users_pincode = pd.DataFrame(top_user_pincode())
    top_users_pincode['State']=top_users_pincode['State'].replace(states)

    return  agg_trans,agg_user,map_trans,df_map_user,top_tran_district,top_trans_pincode,top_users_district,top_users_pincode
    


def sql_query(agg_trans,agg_user,map_trans,df_map_user,top_tran_district,top_trans_pincode,top_users_district,top_users_pincode):
    ...
    mycursor.execute("create table if not exists aggregated_transaction (state	varchar(255),year int,Quarter varchar(2),Transaction_type varchar(255),Transaction_count int,Transaction_amount	float)")
    ...
    mycursor.execute("create table  IF NOT EXISTS agg_user (State varchar(100), Year int, Quarter int, Brands varchar(100), Count int, Percentage float)")
    ...
    mycursor.execute("create table  IF NOT EXISTS map_trans (State varchar(100), Year int, Quarter int, District varchar(100), Count int, Amount float)")
    ...
    mycursor.execute("create table  IF NOT EXISTS top_trans_district (State varchar(100), Year int, Quarter int,District  varchar(100), Transaction_count int, Transaction_amount float)")

    for i,row in top_tran_district.iterrows():
        sql = "INSERT INTO top_trans_district VALUES (%s,%s,%s,%s,%s,%s)"
        mycursor.execute(sql, tuple(row))
        mydb.commit()

    #Creating top_user table
    mycursor.execute("create table  IF NOT EXISTS top_trans_pincode (State varchar(100), Year int, Quarter int, Pincode int,Transaction_count int, Transaction_amount float)")

    for i,row in top_trans_pincode.iterrows():
        sql = "INSERT INTO top_trans_pincode VALUES (%s,%s,%s,%s,%s,%s)"
        mycursor.execute(sql, tuple(row))
        mydb.commit()

    #Creating top_user table
    mycursor.execute("create table  IF NOT EXISTS top_user_district (State varchar(100), Year int, Quarter int, District varchar(100), Registered_users int)")

    for i,row in top_users_district.iterrows():
        sql = "INSERT INTO top_user_district VALUES (%s,%s,%s,%s,%s)"
        mycursor.execute(sql, tuple(row))
        mydb.commit()

    #Creating top_user table
    mycursor.execute("create table  IF NOT EXISTS top_user_pincode (State varchar(100), Year int, Quarter int, Pincode int, Registered_users int)")

    for i,row in top_users_pincode.iterrows():
        sql = "INSERT INTO top_user_pincode VALUES (%s,%s,%s,%s,%s)"
        mycursor.execute(sql, tuple(row))
        mydb.commit()

# agg_trans, agg_user, map_trans, df_map_user, top_tran_district, top_trans_pincode, top_users_district, top_users_pincode = all_data()
# table_creat_insert_queries = sql_query(agg_trans, agg_user, map_trans, df_map_user, top_tran_district, top_trans_pincode, top_users_district, top_users_pincode)
